...
export default function CountdownWidget({ iconOnly = false, compact = false }) {
  ...
  console.log("Goals data:", goals); // Debugging line to inspect type
  ...
  if (
    ...
    !user ||
    goalLoading ||
    !Array.isArray(goals) || // ✅ safer check to avoid "map is not a function"
    goals.length === 0 ||
    dismissed
  )
    return null;
